...
void homelist_onclick(LIBAROMA_CONTROLP list){
	...
	switch (item->id){
		case ID_HOMELIST_FM:{
			if (!aromafm()->cwd){ /* initialize fm by navigating to root */
				libaroma_ctl_bar_set_icon(appbar, NULL, 0, LIBAROMA_CTL_BAR_ICON_DRAWER_TO_ARROW, 0);
				aromafm_loadpath(strdup("/"));
			}
			else { /* switch to last fm fragment and restore appbar text */
				if (!aromafm()->at_root){ /* if not at root, restore tools */
					libaroma_ctl_bar_set_tools(appbar, aromafm()->bar_tools, 0);
				}
				libaroma_ctl_fragment_set_active_window(fragctl, (aromafm()->onmain_list)?ID_FILEMFRAG:ID_FILESFRAG, 0, MLX_ANISWITCH_DURATION, 0, NULL, NULL, NULL);
				aromafm_bar_update((aromafm()->at_root)?aromafm()->base:aromafm_basename(aromafm()->base), aromafm()->dir, LIBAROMA_CTL_BAR_ICON_DRAWER_TO_ARROW);
			}
		} break;
		case ID_HOMELIST_SH:{
			libaroma_ctl_fragment_set_active_window(fragctl, ID_SETTFRAG, 0, MLX_ANISWITCH_DURATION, 0, NULL, NULL, NULL);
			aromafm_bar_update("Shell test", NULL, LIBAROMA_CTL_BAR_ICON_DRAWER_TO_ARROW);
		} break;
		case ID_HOMELIST_DM:/*{
			
		} break;*/
		case ID_HOMELIST_EVT:/*{
			
		} break;*/
		case ID_HOMELIST_RB:/*{
			
		} break;*/
		case ID_HOMELIST_SHD:{
			libaroma_sleep(450);
			libaroma_dialog_confirm("Nope", "Not implemented yet, I should focus instead of adding new things every hour", "Hrm okay, F", NULL, libaroma_colorget(NULL, winmain), LIBAROMA_DIALOG_DIM_PARENT|LIBAROMA_DIALOG_WITH_SHADOW|LIBAROMA_DIALOG_CANCELABLE);
		} break;
		case ID_HOMELIST_BYE:{
...
void aromafm_list_onclick(LIBAROMA_CONTROLP list){	
	...
	if (aromafm()->selecting){
		...
		if (item->tag!=NULL){ //check for file path
			...
			printf("MLX: item path is %s (%p)\n", file_path, file_path);
			if (!strcmp(file_path, "..")){ //go one level up
				printf("MLX: should go up\n");
				aromafm_goup();
				return;
			}
			printf("MLX: checking if tag is absolute or relative link\n");
			if (file_path[0]!='/'){
				printf("MLX: relative link, building full path\n");
				char *tmp_fpath = file_path;
				int fp_len=strlen(aromafm()->cwd)+strlen(tmp_fpath)+1;
				file_path=malloc(fp_len);
				snprintf(file_path, fp_len, "%s%s", aromafm()->cwd, tmp_fpath);
			}
...
void sidelist_onclick(LIBAROMA_CONTROLP list){
	...
	switch (item->id){
		...
		case ID_SIDELIST_ABOUT:
			libaroma_sleep(500);
			libaroma_dialog_confirm("LumiaTest", 
									"Libaroma by:\n"
									"<i>Ahmad Amarullah</i>\n"
									"(<u>amarullz@xda</u>)\n"
									"This app by:\n"
...
void aromafm_navbar_onclick(byte button_id){
	switch (button_id){
		case FM_NAVBAR_BACK: /* send back key event */
			...
			break;
		case FM_NAVBAR_HOME:{
			if (libaroma_wm_get_active_window()!=winmain){ /* if dialog shown, switch to main and free dialog */
				libaroma_msg_post(LIBAROMA_MSG_EXIT, 1, 0, 0, 0, NULL); /* trigger dialog close */
			}
			else if (libaroma_ctl_fragment_get_active_window_id(fragctl)!=ID_HOMEFRAG){
