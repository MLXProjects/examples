...
byte mlx_navbar_draw(){
	...
	if (res&LIBAROMA_RIPPLE_REDRAW || _navbar.forceredraw){
		if (_navbar.forceredraw) _navbar.forceredraw=0; /* use as one-time flag */
		_navbar.redraw=1;
		int ripple_i = 0;
		int ripple_p = 0;
...
byte mlx_navbar_msg(LIBAROMA_WMP wm, LIBAROMA_MSGP msg){
	if (msg->y > (libaroma_wm()->y+libaroma_wm()->h)){ /* touch behind of window workspace */
		int x=msg->x;
		int y=msg->y-(libaroma_wm()->y+libaroma_wm()->h); /* take wm offset into account */
		if (x>libaroma_fb()->w-(_navbar.button_width)){ /* third button */
			if (!_navbar.updating) _navbar.updated_btn=FM_NAVBAR_MENU;
		}
		else if (x>_navbar.button_width){ /* second button */
