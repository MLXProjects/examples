...
void aromafm_pathcleanup(){
	if (_aromafm.cwd!=NULL) {
		//printf("freeing cwd@%p\n", _aromafm.cwd);
...
byte aromafm_loadpath(char *path){
	...
	//libaroma_ctl_progress_type(busyprog, LIBAROMA_CTL_PROGRESS_INDETERMINATE);
	...
	if (!_aromafm.at_root){
		...
		//printf("loadpath saved orig dir@%p\n", _aromafm.dir);
		/*dpath=*/
		dirname(_aromafm.dir);
	...
	//printf("loadpath saved cwd@%p, base@%p, dir@%p\n", _aromafm.cwd, _aromafm.base, _aromafm.dir);
	/* update appbar */
	aromafm_bar_update((_aromafm.at_root)?_aromafm.base:aromafm_basename(_aromafm.base), _aromafm.dir, 0);
	/* show target fragment */
	/* TODO: research about activate fragment rendering performance issues if drawing LONG (>1k files) list */
	libaroma_ctl_fragment_set_active_window(fragctl, target_id, 0, MLX_ANISWITCH_DURATION, 0, NULL, NULL, NULL);
	//libaroma_ctl_progress_type(busyprog, LIBAROMA_CTL_PROGRESS_DETERMINATE);
...
byte aromafm_exit(){
	...
	aromafm_pathcleanup();
	libaroma_ctl_bar_set_tools(appbar, NULL, 0);
	libaroma_ctl_bar_tools_free(aromafm()->bar_tools);
	aromafm()->bar_tools=NULL;
	libaroma_canvas_free(aromafm()->up_arrow);
	libaroma_canvas_free(aromafm()->folder_cv);
	libaroma_canvas_free(aromafm()->file_cv);
	aromafm()->up_arrow=NULL;
	aromafm()->folder_cv=NULL;
	aromafm()->file_cv=NULL;
}
