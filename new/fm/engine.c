...
int aui_fetch(char * path, LIBAROMA_CONTROLP list){
	...
	if (n > 0) {
		while (n--) {
			...
			char * full_path = NULL;
			aui_setpath(&full_path, aromafm()->cwd, dname, ((dtype==4)||dtype==24)?1:0);
				...
				switch (dtype){
					case 4:{ //-- DIR
						...
						item = libaroma_listitem_check(list, 1, 0,
													dname, perms, aromafm()->folder_cv, LIBAROMA_LISTITEM_CHECK_HAS_SUBMENU|LIBAROMA_LISTITEM_WITH_SEPARATOR|LIBAROMA_LISTITEM_SEPARATOR_TEXTALIGN, -1);	
					...
					case 8:{ //-- FILE
						...
						snprintf(perms, 128, "<align=right>%s", desc);
						item = libaroma_listitem_check(list, 1, 0,
													dname, perms, aromafm()->file_cv, LIBAROMA_LISTITEM_CHECK_HAS_SUBMENU|LIBAROMA_LISTITEM_WITH_SEPARATOR|LIBAROMA_LISTITEM_SEPARATOR_TEXTALIGN, -1);	
					...
					case 28:{ //-- LINK DIR / FILE
						...
						item = libaroma_listitem_check(list, 1, 0,
													dname, desc, (dtype==28)?aromafm()->file_cv:aromafm()->folder_cv, LIBAROMA_LISTITEM_CHECK_HAS_SUBMENU|LIBAROMA_LISTITEM_WITH_SEPARATOR|LIBAROMA_LISTITEM_SEPARATOR_TEXTALIGN, 
													-1);
						item->tag=(voidp) buf;
						printf("Found symlink %s with desc %s and buf %s\n", dname, desc, buf);
						/* customization starts here */
					} break;
					#endif /*_WIN32*/
					case 10:{ //-- ERROR LINK
						item = libaroma_listitem_check(list, 1, 0,
													dname, "Broken link", aromafm()->file_cv, LIBAROMA_LISTITEM_CHECK_HAS_SUBMENU|LIBAROMA_LISTITEM_WITH_SEPARATOR|LIBAROMA_LISTITEM_SEPARATOR_TEXTALIGN, -1);
