	...
	do {
		...
		if (msg.msg==LIBAROMA_MSG_TOUCH || msg.msg==LIBAROMA_MSG_MOUSE){
			...
			if ((isdrawing || msg.state==1) && 
				...
				msg.x-=image->x;
				msg.y-=image->y;
				//logi("cur color = %s", (msg.key==LIBAROMA_HID_RMOUSE_KEYCODE)?(sel_second?"primary - right click":"secondary - right click"):(sel_second?"secondary - left click":"primary - left click"));
				word curcolor = (msg.key==LIBAROMA_HID_RMOUSE_KEYCODE)?(sel_second?pri_cl:sec_cl):(sel_second?sec_cl:pri_cl);
