...
#define CUSTITEM_FORCE_LEFT_TEXT	0x12
#define CUSTITEM_SMALL				0x16
#define custitem(icon, text, flags) \
	custitem_ex(icon, text, NULL, NULL, NULL, NULL, NULL, flags)
...
LIBAROMA_CANVASP custitem_ex
	...
	if (!bg_color) bg_color=main_bgcolor;
	if (!accent) accent=main_accent;
	...
	if (extra) {
		...
		LIBAROMA_TEXT extratxt=libaroma_text(extra, accent, textw, extraflags, 0);
		texty=(cv->h/2)-texth;
		int extray=(cv->h/2);//+libaroma_text_height(extratxt);
		if (debug) printf("Drawing extra text \"%s\" at x: %d and y: %d\n", extra, textx, extray);
		libaroma_draw_text(cv, extra, textx, extray, accent, textw, extraflags|((isright&&!(flags&CUSTITEM_FORCE_LEFT_TEXT))?LIBAROMA_TEXT_RIGHT:0), NULL);
	...
	else libaroma_draw_text(cv, text, textx, texty, accent, textw, textflags|((isright&&!(flags&CUSTITEM_FORCE_LEFT_TEXT))?LIBAROMA_TEXT_RIGHT:0), NULL);
...
int custitem_get_icon_x(int ctlwidth, byte isright){	
	int circlex=libaroma_dp(5);
	int circlesz=libaroma_dp(80);
	int iconx=libaroma_dp(5);	
	int iconsz=libaroma_dp(28);
