...
typedef struct{  
	...
	word override_color;
	word orig_override_color;
	byte animating;
	byte status;
	byte alpha;
	int y;
	LIBAROMA_WINDOWP notif_pulldown;
	int pulldown_y;
...
char *getcurtime(/*byte show_seconds*/){	
	...
	char curtime[16];	
    snprintf(curtime, 16, settings()->show_seconds?"%02d:%02d:%02d":"%02d:%02d", 
           current_time->tm_hour,
           current_time->tm_min,
           (!settings()->show_seconds)?NULL:current_time->tm_sec);
...
char *getcurbatt(){
    ...
    char str3[16] = "+";
	const char nl[4] = "\n";
    char* capacityfile = "/sys/class/power_supply/battery/capacity";
    char* statusfile = "/sys/class/power_supply/battery/status";
    FILE *fp1 = fopen(statusfile, "r");
    if (fp1 != NULL){
		fgets(status, 16, fp1);
		fclose(fp1);
		strtok(str, nl);
		char *statusp = malloc(16);
		strcpy(statusp, status);
		if (!strcmp(statusp, "Charging")){
			printf("Device is charging!\n");
			strcat(str, str3);
		}
    }
 
    FILE *fp = fopen(capacityfile, "r");
    if (fp == NULL){
        printf("STATUSBAR: Could not open file %s",capacityfile);
		char *string = malloc(16);
		strcpy(string, "lol");
		return string;
    }
...
byte statusbar_draw(){
	...
	libaroma_draw_text(
		...
		getcurtime(),
		0, _statusbar.y, settings()->sb_text_color, _statusbar.temp_canvas->w, 
		LIBAROMA_FONT(0,2)|
	...
	libaroma_draw_text(
		...
		_statusbar.title,
		0, _statusbar.y, settings()->sb_text_color, _statusbar.temp_canvas->w, 
		LIBAROMA_FONT(settings()->title_italic?FONT_LIGHT_ITALIC:FONT_LIGHT,2)|
		LIBAROMA_TEXT_SINGLELINE|
	...
	libaroma_draw_ex(_statusbar.canvas, _statusbar.temp_canvas, 0, 0/*_statusbar.y*/, 0, 0, 
					 _statusbar.temp_canvas->w, _statusbar.temp_canvas->h, 0, 0xFF);
	//if (!_statusbar.animating) libaroma_sleep(500);
	//printf("Alpha: %d\n", _statusbar.alpha);
	if (_statusbar.notif_pulldown && _statusbar.pulldown_y>0){
		printf("Drawing pulldown over statusbar at y=%d!\n", _statusbar.notif_pulldown->h-_statusbar.pulldown_y);
		libaroma_draw_ex(_statusbar.canvas, _statusbar.notif_pulldown->dc, 0, 0, 0, 
				_statusbar.notif_pulldown->h-_statusbar.pulldown_y-_statusbar.canvas->h, 
				_statusbar.canvas->w, _statusbar.canvas->h, 0, 0xFF);
	}
	else if (_statusbar.override_color != _statusbar.orig_override_color) 
		_statusbar.override_color=_statusbar.orig_override_color;
}
...
void statusbar_init(word override_bg, char *title){
	printf("STATUSBAR: Starting with override color %ld!\n", override_bg);
	_statusbar.orig_override_color = override_bg;
...
void statusbar_close(){	
	...
	//while (_statusbar.status == SB_CLOSING) { libaroma_sleep(50); }
	statusbar_animate();
	libaroma_wm_set_ui_thread(NULL);
